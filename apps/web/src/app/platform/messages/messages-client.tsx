'use client';
/**
 * Super-admin ⇄ rider chat. Left column = conversations + a "start new chat"
 * picker over the whole rider roster; right pane = the selected thread with
 * chat bubbles and a composer. The open thread is polled every ~4s; sending is
...
export function PlatformMessagesClient({
  ...
  const loadThread = useCallback(async (id: string, silent = false) => {
    ...
    try {
      const r = await fetch(`/api/platform/messages/${id}`, { cache: 'no-store' });
  ...
  async function send() {
    ...
    setSending(true);
    try {
      const res = await fetch(`/api/platform/messages/${activeId}`, {
        method: 'POST',
              ...
              <div className="max-h-56 space-y-1 overflow-y-auto">
                {startableRiders.length === 0 ? (
                  <p className="px-1 py-2 text-xs text-muted-foreground">
                    No riders match — or they all have a conversation already.
                  </p>
                ) : (
            ...
            {sortedConversations.length === 0 ? (
              <div className="p-4">
                <EmptyState
                  ...
                  description="Start a chat with a rider using the New button."
                />
              </div>
            ) : (
            ...
            <div className="border-t p-3">
              <div className="flex items-end gap-2">
                <Textarea
                  ...
                  placeholder="Message as Platform Ops…"
  ...
  return (
    <div className={`flex ${fromRider ? 'justify-start' : 'justify-end'}`}>
      ...
      >
        ...
        >
          {fromRider ? message.senderName ?? 'Rider' : message.senderName ?? 'Platform Ops'}
        </div>
