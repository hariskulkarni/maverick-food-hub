...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return { title: r ? `Staff sign-in · ${r.name}` : 'Staff sign-in' };
}

export default async function RestaurantStaffLoginPage({
  params
}: {
      ...
      <div className="container py-6 md:py-10 max-w-md">
        ...
        <div className="text-center mb-6 reveal">
          ...
          <p className="mt-1.5 text-sm text-muted-foreground">
            Staff sign-in for {restaurant.name}
          </p>
        </div>

        <Card className="overflow-hidden">
          <CardContent className="p-6">
            <StaffLoginClient slug={restaurant.slug} restaurantName={restaurant.name} />
          </CardContent>
        </Card>

        <p className="text-center text-xs text-muted-foreground mt-6">
          Customer?{' '}
          <Link href={`/r/${restaurant.slug}/login`} className="text-primary underline">
            Sign in at /r/{restaurant.slug}/login
          </Link>
        </p>
      </div>
    </div>
